...
<?php $this->uri->segment(1,'write')=='modify' ? $write_type="수정" : $write_type="등록"; ?>
<div class="wrap03">
<section class="write">
<h2><?php echo html_escape(element('board_name', element('board', $view))); ?> 게시물 <?php echo html_escape($write_type); ?>하기 <span><a href="<?php echo base_url('/board/'.element('board_key', $view)) ?>">목록보기 ></a></span></h2>
    ...
    ?>
        ...
        <div style="height:30px;">
            ...
            <div class="search">
                <input type="text" class="per95"  name="post_nickname" id="post_nickname" value="<?php echo set_value('post_nickname', element('post_nickname', element('post', $view))); ?>">
            </div>
        </div>
            
            <?php if ($this->member->is_member() === false) { ?>
            <div style="height:30px;">
                <div class="title" style='CLEAR:BOTH;'>비번</div>
                <div class="search"   >
                    <input type="password" class="per95"  name="post_password" id="post_password" >
                </div>
            </div>
            <?php } ?>
        ...
        <?php } else { ?>
            <!-- 글쓰기의 작성자 정보 -->
            <figure>
            <img src="<?php echo base_url('/assets/mobi/images/setting.png') ?>" alt="setting">
            <figcaption><?php echo $this->member->item('mem_username'); ?> <span><?php echo date("Y.m.d"); ?></span></figcaption>
            </figure>
        ...
        <!--제목영역 영역-->
        <div style="height:30px;">
        <div class="title" style='CLEAR:BOTH;'>제목</div>
        <div class="search"  id="title_txt" >
        ...
        <div style="height:30px;">
            <div class="title" style='CLEAR:BOTH;'>옵션</div>
        ...
        <div class="form-group mb20">
            ...
            <?php echo display_dhtml_editor('post_content', set_value('post_content', element('post_content', element('post', $view))), $classname = 'content', $is_dhtml_editor = element('use_dhtml', element('board', $view)), $editor_type = $this->cbconfig->item('post_editor_type')); ?>
        </div>
          <?php if ( ! element('use_dhtml', element('board', $view)) AND (element('post_min_length', element('board', $view)) OR element('post_max_length', element('board', $view)))) { ?>
            <div class="well well-sm" style="margin-bottom:15px;">
                현재 <strong><span id="char_count">0</span></strong> 글자이며,
        ...
        ?>
            <div class="filebox bs3-primary preview-image">
                <label for="<?php echo $file_column; ?>">업로드 <?php echo $i+1; ?></label>
                <input class="upload-name" value="선택된 파일이 없습니다." disabled="disabled" >
                <input type="file" id="<?php echo $file_column; ?>" name="<?php echo $file_column; ?>" class="input upload-hidden"> 
                <?php if ($download_link) { ?>
                    <br><a href="<?php echo $download_link; ?>"><?php echo html_escape(element('pfi_originname', element($i, element('file', $view)))); ?></a>
                    <label for="<?php echo $del_column; ?>" disabled="disabled" >
        ...
        <?php } ?>
            <div class="table-bottom text-center mt20">
                ...
                <div class="write_btn">                
                <h3><a href="javascript:$('#fwrite').submit();">게시물 <?php echo html_escape($write_type); ?>하기</a></h3>
                </div>
            </div>
...
$(function() {
    $('#fwrite').validate({
        rules: {
            post_title: {required :false, minlength:2, maxlength:60},
            post_content : {<?php echo (element('use_dhtml', element('board', $view))) ? 'required_' . $this->cbconfig->item('post_editor_type') : 'required'; ?> : false }
<?php if (element('is_post_name', element('post', $view))) { ?>
            , post_nickname: {required :false, minlength:2, maxlength:20}
            , post_email: {required :false, email:true}
<?php } ?>
<?php if ($this->member->is_member() === false) { ?>
            , post_password: {required :false, minlength:4, maxlength:100}
<?php if ($this->cbconfig->item('use_recaptcha')) { ?>
