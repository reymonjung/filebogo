...
<!-- <link rel="stylesheet" type="text/css" href="<?php echo element('view_skin_url', $layout); ?>/css/style.css" /> -->
<link rel="stylesheet" type="text/css" href="<?php echo element('layout_skin_url', $layout); ?>/css/style.css" />
<!-- <link rel="stylesheet" type="text/css" href="//fonts.googleapis.com/earlyaccess/nanumgothic.css" /> -->
<link rel="stylesheet" type="text/css" href="//maxcdn.bootstrapcdn.com/font-awesome/4.5.0/css/font-awesome.min.css" />
<!-- <link rel="stylesheet" type="text/css" href="//ajax.googleapis.com/ajax/libs/jqueryui/1.11.4/themes/ui-lightness/jquery-ui.css" /> -->

<link rel="stylesheet" type="text/css" href="<?php echo base_url('assets/mobi/css/import.css'); ?>" />
...
<!-- 헤더영역 -->
    <header> 
      ...
      <nav class="main" >
        <ul>
            <?php
            $segment_arr=explode("_",element('board_key', $view));
            $menuhtml = '';
                ...
                if (element(0, $menu)) {
                    foreach (element(0, $menu) as $mkey => $mval) {
                        ...
                        if (false) {    
                            ...
                            if($segment_arr[0])
                                if(strpos($mval['men_link'],$segment_arr[0]) !==false) $click_bar = 'id="click_bar"';
    ...
    if(element($men_id, $menu)){

        $segment_arr[0]!="community" ? $submenuClass='submenu_01':$submenuClass='sub';
        
        $this->uri->segment(1)!="board"  ? $hideClass='hide':$hideClass='';
        $submenuhtml='';
        echo '<section class="'.$submenuClass.' '.$hideClass.'">
            <h4>서브메뉴 영역</h4>
            <nav>
                <ul>
            ';
        ...
        foreach (element($men_id, $menu) as $skey => $sval) {
            ...
            $submenuhtml .= '<li data-id="menu'.$i.'"><a href="' . element('men_link', $sval) . '" ' . element('men_custom', $sval);
            if (element('men_target', $sval)) {
                $menuhtml .= ' target="' . element('men_target', $sval) . '"';
            }
            $submenuhtml .= ' title="' . html_escape(element('men_name', $sval)) . '">' . html_escape(element('men_name', $sval)) . '</a></li>';
