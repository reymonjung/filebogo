...
class Videoplayer extends CI_Controller
{
    ...
    function get_video($vid = '',$width='',$height='')
    {
        ...
        if (element('type', $video) === 'file') { //JWPLAYER
            ...
            } elseif (element('type', $video) === 'tumblr') {

                $show = '<iframe src="'.$vid.'" style="display:block;background-color:transparent;overflow:hidden" class="embed_iframe tumblr_video_iframe" scrolling="no" frameBorder="0" data-can-gutter data-can-resize data-width="100%" data-height="100%" width="100%" height="100%" allowfullscreen mozallowfullscreen webkitallowfullscreen></iframe>';
    ...
    function get_video_info($video_url)
    {
        ...
        if (element('host', $info) === 'youtu.be') { //유튜브
            ...
            $video['vid'] = element('vid', $play);
        } elseif (element('host', $info) === 'www.tumblr.com' || element('host', $info) === 'tumblr.com') { // 텀블러
            $video['type'] = 'tumblr';            
        }
