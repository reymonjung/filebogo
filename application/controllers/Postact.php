...
class Postact extends CB_Controller
{
     ...
     * 엑박 URL을 다른 URL 로 변경
     */
    public function img_url_change($brd_id,$limit)
    {
        ...
        $post_link_array  = $this->Post_link_model->get('', '', $linkwhere, 20, $limit, 'pln_id', 'DESC');
        $return_src="";
        foreach ($post_link_array as $key => $value) {
                 if(urlExitsCheck(element('pln_url', $value))) {
                    $return_src='/img_url_header.php?url='.urlencode(element('pln_url', $value)).'&filename=photo_'.$limit;
                    break;
                }
        }
