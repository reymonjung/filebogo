...
class Board_post extends CB_Controller
{
    ...
    public function lists($brd_key = '')
    {
        ...
        $segment_arr=explode("_",$this->uri->segment(2,'webtoon'));

        $list_skin_file = element('use_gallery_list', element('board', $list)) ? 'gallerylist_'.$segment_arr[0] : 'list_'.$segment_arr[0];
    ...
    public function post($post_id = 0, $print = false)
    {
        ...
        if ($print === false) {
            $segment_arr=explode("_",element('brd_key', $board));
            // 이벤트가 존재하면 실행합니다
            ...
            $this->layout = element('layout_skin_file', element('layout', $view));
            if ($show_list_from_view) {
                $list_skin_file = 'list_extra_video';
